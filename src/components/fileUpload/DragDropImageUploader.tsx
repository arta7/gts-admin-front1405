...
const DrageDropImageUploader = ({ title, fileUploadButtonTitle, value, onChange }: { title: string, fileUploadButtonTitle: string, value: Array<any>, onChange: (val: Array<any>) => any }) => {
  ...
  const handleChange = (files: any) => {
    ...
    if (files && files.length) {
      for (let index = 0; index < files.length; index++) {
        ...
        if (value.findIndex((f: any) => f.name === file.name) === -1) {
          newFiles.push(file);
        }
      }
      const promises = newFiles.map((item: any) => readFile(item));
...
export default DrageDropImageUploader;
